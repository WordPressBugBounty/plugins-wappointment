...
{
    use SoftDeletes;
    use \Wappointment\Models\CanBook;
    use \Wappointment\Models\CanBookLegacy;
    use \Wappointment\Models\CanGetCustomFieldValue;
    protected $table = 'wappo_clients';
    public $generatingOrder = \true;
    protected $fillable = ['name', 'email', 'options'];
    protected $casts = ['options' => 'array'];
    protected $appends = ['avatar'];
    public function appointments()
    {
        return $this->hasMany(\Wappointment\Models\Appointment::class);
    }
    public function hasActiveBooking($staff_id)
    {
        $start_at_string = Carbon::now('UTC')->format(WAPPOINTMENT_DB_FORMAT);
        $appointments_query = \Wappointment\Models\Appointment::where('client_id', $this->id)->where('start_at', '>=', $start_at_string);
