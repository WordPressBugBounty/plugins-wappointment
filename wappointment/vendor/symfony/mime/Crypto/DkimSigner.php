 ...
 */
namespace WappoVendor\Symfony\Component\Mime\Crypto;

use WappoVendor\Symfony\Component\Mime\Exception\InvalidArgumentException;
use WappoVendor\Symfony\Component\Mime\Exception\RuntimeException;
use WappoVendor\Symfony\Component\Mime\Header\UnstructuredHeader;
use WappoVendor\Symfony\Component\Mime\Message;
use WappoVendor\Symfony\Component\Mime\Part\AbstractPart;
/**
...
final class DkimSigner
{
    ...
    public function sign(Message $message, array $options = []): Message
    {
        ...
        $params = ['v' => '1', 'q' => 'dns/txt', 'a' => $options['algorithm'], 'bh' => base64_encode($bodyHash), 'd' => $this->domainName, 'h' => implode(': ', $signedHeaderNames), 'i' => '@' . $this->domainName, 's' => $this->selector, 't' => time(), 'c' => $options['header_canon'] . '/' . $options['body_canon']];
