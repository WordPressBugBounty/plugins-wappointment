...
namespace WappoVendor\GuzzleHttp\Handler;

use WappoVendor\GuzzleHttp\Exception\ConnectException;
use WappoVendor\GuzzleHttp\Exception\RequestException;
use WappoVendor\GuzzleHttp\Promise\FulfilledPromise;
use WappoVendor\GuzzleHttp\Promise\PromiseInterface;
use WappoVendor\GuzzleHttp\Psr7;
use WappoVendor\GuzzleHttp\TransferStats;
use WappoVendor\GuzzleHttp\Utils;
use WappoVendor\Psr\Http\Message\RequestInterface;
use WappoVendor\Psr\Http\Message\ResponseInterface;
...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return \WappoVendor\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $headers = \WappoVendor\GuzzleHttp\headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \WappoVendor\GuzzleHttp\Promise\rejection_for($ex);
            }
        }
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \WappoVendor\GuzzleHttp\normalize_header_keys($headers);
    ...
    private function add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\WappoVendor\GuzzleHttp\is_host_in_noproxy($request->getUri()->getHost(), $value['no'])) {
    ...
    private function add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \WappoVendor\GuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \WappoVendor\GuzzleHttp\debug_resource($value);
