...
namespace WappoVendor\Sabre\VObject\Recur;
...
class RRuleIterator implements Iterator
{
    ...
    protected function nextMonthly()
    {
        ...
        $this->currentDate = $this->currentDate->setDate((int) $this->currentDate->format('Y'), (int) $this->currentDate->format('n'), (int) $occurrence);
    }
    /**
    ...
    protected function nextYearly()
    {
        ...
        if (empty($this->byMonth)) {
            ...
            if (null !== $this->byWeekNo) {
                // byWeekNo is an array with values from -53 to -1, or 1 to 53
                $dayOffsets = [];
                if ($this->byDay) {
            ...
            if (null !== $this->byYearDay) {
                // byYearDay is an array with values from -366 to -1, or 1 to 366
                $dayOffsets = [];
                if ($this->byDay) {
                    foreach ($this->byDay as $byDay) {
                        $dayOffsets[] = $this->dayMap[$byDay];
        ...
        if ($this->byDay || $this->byMonthDay) {
            ...
            while (\true) {
                ...
                $this->currentDate = $this->currentDate->setDate((int) $currentYear, (int) $currentMonth, (int) $currentDayOfMonth);
                // To prevent running this forever (better: until we hit the max date of DateTimeImmutable) we simply
                // stop at 9999-12-31. Looks like the year 10000 problem is not solved in php ....
                if ($this->currentDate->getTimestamp() > self::dateUpperLimit) {
                    $this->currentDate = null;
            ...
            $this->currentDate = $this->currentDate->setDate((int) $currentYear, (int) $currentMonth, (int) $occurrence);
            return;
        } else {
        ...
        foreach ($rrule as $key => $value) {
            ...
            switch ($key) {
                case 'FREQ':
                    ...
                    if (!in_array($value, ['secondly', 'minutely', 'hourly', 'daily', 'weekly', 'monthly', 'yearly'])) {
    ...
    protected function getMonthlyOccurrences()
    {
        ...
        if ($this->byDay) {
            foreach ($this->byDay as $day) {
                ...
                if (strlen($day) > 2) {
                    ...
                    } else if (isset($dayHits[count($dayHits) + $offset])) {
        ...
        if ($this->byMonthDay) {
            foreach ($this->byMonthDay as $monthDay) {
                ...
                if ($monthDay > $startDate->format('t') || $monthDay < 0 - $startDate->format('t')) {
                    continue;
                }
                if ($monthDay > 0) {
