...
namespace WappoVendor\Sabre\VObject\Property\ICalendar;
...
use WappoVendor\Sabre\VObject\TimeZoneUtil;
/**
 * DateTime property.
 *
...
class DateTime extends Property
{
    ...
    {
        return !$this->hasTime() || !isset($this['TZID']) && \false === strpos($this->getValue(), 'Z');
    }
    /**
    ...
    public function setDateTimes(array $dt, $isFloating = \false)
    {
        ...
        if ($this->hasTime()) {
            ...
            foreach ($dt as $d) {
                if ($isFloating) {
                    $values[] = $d->format('WappoVendor\Ymd\THis');
                    continue;
                }
                if (is_null($tz)) {
                    $tz = $d->getTimeZone();
                ...
                if ($isUtc) {
                    $values[] = $d->format('WappoVendor\Ymd\THis\Z');
                } else {
                    $values[] = $d->format('WappoVendor\Ymd\THis');
                }
            }
    ...
    public function getJsonValue()
    {
        ...
        return array_map(function (DateTimeInterface $dt) use ($hasTime, $isUtc) {
            if ($hasTime) {
