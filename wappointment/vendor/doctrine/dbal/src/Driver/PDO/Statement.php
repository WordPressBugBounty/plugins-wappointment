...
namespace WappoVendor\Doctrine\DBAL\Driver\PDO;

use WappoVendor\Doctrine\DBAL\Driver\Exception as ExceptionInterface;
...
final class Statement implements StatementInterface
{
    ...
    public function bindParam($param, &$variable, $type = ParameterType::STRING, $length = null, $driverOptions = null): bool
    {
        ...
        $type = $this->convertParamType($type);
        try {
            return $this->stmt->bindParam($param, $variable, $type, $length ?? 0, ...array_slice(func_get_args(), 4));
        } catch (PDOException $exception) {
            throw Exception::new($exception);
        }
    }
