...
use WappoVendor\Doctrine\DBAL\Types\BlobType;
use WappoVendor\Doctrine\DBAL\Types\PhpIntegerMappingType;
...
{
    /** @var bool */
    private $useBooleanTrueFalseStrings = \true;
    /** @var string[][] PostgreSQL booleans literals */
    private $booleanLiterals = ['true' => ['t', 'true', 'y', 'yes', 'on', '1'], 'false' => ['f', 'false', 'n', 'no', 'off', '0']];
    /**
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->changedColumns as $columnDiff) {
            ...
            if ($columnDiff->hasChanged('default') || $this->typeChangeBreaksDefaultValue($columnDiff)) {
                $defaultClause = $column->getDefault() === null ? ' DROP DEFAULT' : ' SET' . $this->getDefaultValueDeclarationSQL($column->toArray());
            ...
            if ($columnDiff->hasChanged('comment') || $columnDiff->fromColumn !== null && $oldComment !== $newComment) {
                $commentsSQL[] = $this->getCommentOnColumnSQL($diff->getName($this)->getQuotedName($this), $column->getQuotedName($this), $newComment);
            }
