...
use WappoVendor\Doctrine\DBAL\Driver\API\SQLite\UserDefinedFunctions;
...
class SqlitePlatform extends AbstractPlatform
{
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/4753', 'SqlitePlatform::getNowExpression() is deprecated. Generate dates within the application.');
        switch ($type) {
            case 'time':
                return 'time(\'now\')';
            case 'date':
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pulls/1519', 'SqlitePlatform::prefersIdentityColumns() is deprecated.');
        return \true;
    }
    /**
     * {@inheritDoc}
    ...
    protected function _getCreateTableSQL($name, array $columns, array $options = [])
    {
        ...
        if (isset($options['alter']) && $options['alter'] === \true) {
            return $query;
        }
    ...
    {
        return $fixed ? $length > 0 ? 'CHAR(' . $length . ')' : 'CHAR(255)' : ($length > 0 ? 'VARCHAR(' . $length . ')' : 'TEXT');
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function getListTableConstraintsSQL($table)
    {
        ...
        return sprintf("SELECT sql FROM sqlite_master WHERE type='index' AND tbl_name = %s AND sql NOT NULL ORDER BY name", $this->quoteStringLiteral($table));
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return 'SELECT name FROM sqlite_master' . " WHERE type = 'table'" . " AND name != 'sqlite_sequence'" . " AND name != 'geometry_columns'" . " AND name != 'spatial_ref_sys'" . ' UNION ALL SELECT name FROM sqlite_temp_master' . " WHERE type = 'table' ORDER BY name";
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4749', 'SqlitePlatform::getName() is deprecated. Identify platforms by their class.');
        return 'sqlite';
    }
    /**
     * {@inheritDoc}
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4510', 'SqlitePlatform::getReservedKeywordsClass() is deprecated,' . ' use SqlitePlatform::createReservedKeywordsList() instead.');
    ...
    protected function getPreAlterTableIndexForeignKeySQL(TableDiff $diff)
    {
        if (!$diff->fromTable instanceof Table) {
            throw new Exception('Sqlite platform requires for alter table the table diff with reference to original table schema');
        }
        $sql = [];
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/4805', 'SqlitePlatform::canEmulateSchemas() is deprecated.');
        return \true;
    }
    /**
     * {@inheritDoc}
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->removedColumns as $columnName => $column) {
            ...
            unset($columns[$columnName], $oldColumnNames[$columnName], $newColumnNames[$columnName]);
        }
        ...
        if (!$this->onSchemaAlterTable($diff, $tableSql)) {
            ...
            if ($newName !== \false) {
                $sql[] = sprintf('ALTER TABLE %s RENAME TO %s', $newTable->getQuotedName($this), $newName->getQuotedName($this));
            }
    ...
    private function replaceColumn($tableName, array $columns, $columnName, Column $column): array
    {
        ...
        $index = array_search($columnName, $keys, \true);
        if ($index === \false) {
    ...
    private function getSimpleAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->changedColumns as $oldColumnName => $columnDiff) {
            if ($columnDiff->fromColumn === null || !$columnDiff->column->getAutoincrement() || !$columnDiff->column->getType() instanceof Types\IntegerType) {
                continue;
            }
