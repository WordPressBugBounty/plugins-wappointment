...
use WappoVendor\Doctrine\DBAL\Types\BinaryType;
...
class OraclePlatform extends AbstractPlatform
{
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/4753', 'OraclePlatform::getNowExpression() is deprecated. Generate dates within the application.');
        switch ($type) {
            case 'date':
            case 'time':
            case 'timestamp':
    ...
    {
        return '(' . $value1 . '-' . $this->getBitAndComparisonExpression($value1, $value2) . '+' . $value2 . ')';
    }
    /**
     * {@inheritDoc}
     */
    public function getCreatePrimaryKeySQL(Index $index, $table): string
    {
        ...
        return 'ALTER TABLE ' . $table . ' ADD CONSTRAINT ' . $index->getQuotedName($this) . ' PRIMARY KEY (' . $this->getIndexFieldDeclarationListSQL($index) . ')';
    }
    /**
     * {@inheritDoc}
     *
    ...
    {
        return $fixed ? $length > 0 ? 'CHAR(' . $length . ')' : 'CHAR(2000)' : ($length > 0 ? 'VARCHAR2(' . $length . ')' : 'VARCHAR2(4000)');
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function getListSequencesSQL($database)
    {
        ...
        return 'SELECT sequence_name, min_value, increment_by FROM sys.all_sequences ' . 'WHERE SEQUENCE_OWNER = ' . $database;
    }
    /**
     * {@inheritDoc}
     */
    ...
    public function getCreateAutoincrementSql($name, $table, $start = 1)
    {
        ...
        $idx = new Index($autoincrementIdentifierName, [$quotedName], \true, \true);
        $sql[] = "DECLARE\n  constraints_Count NUMBER;\nBEGIN\n  SELECT COUNT(CONSTRAINT_NAME) INTO constraints_Count\n    FROM USER_CONSTRAINTS\n   WHERE TABLE_NAME = '" . $unquotedTableName . "'\n     AND CONSTRAINT_TYPE = 'P';\n  IF constraints_Count = 0 OR constraints_Count = '' THEN\n    EXECUTE IMMEDIATE '" . $this->getCreateConstraintSQL($idx, $quotedTableName) . "';\n  END IF;\nEND;";
        $sequenceName = $this->getIdentitySequenceName($tableIdentifier->isQuoted() ? $quotedTableName : $unquotedTableName, $nameIdentifier->isQuoted() ? $quotedName : $unquotedName);
...
END;';
    ...
    public function getDropAutoincrementSql($table)
    {
        ...
        return ['DROP TRIGGER ' . $autoincrementIdentifierName, $this->getDropSequenceSQL($identitySequenceName), $this->getDropConstraintSQL($autoincrementIdentifierName, $table->getQuotedName($this))];
    }
    /**
...
SQL
, $colCommentsTableName, $colCommentsOwnerCondition, $tabColumnsTableName, $table, $tabColumnsOwnerCondition);
    }
    /**
     * {@inheritDoc}
     */
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->changedColumns as $columnDiff) {
            ...
            if ($column->getType() instanceof BinaryType && $columnDiff->hasChanged('fixed') && count($columnDiff->changedProperties) === 1) {
                continue;
            }
            ...
            $commentsSQL[] = $this->getCommentOnColumnSQL($diff->getName($this)->getQuotedName($this), $column->getQuotedName($this), $this->getColumnComment($column));
        }
        if (count($fields) > 0) {
        ...
        foreach ($diff->renamedColumns as $oldColumnName => $column) {
            ...
            $sql[] = 'ALTER TABLE ' . $diff->getName($this)->getQuotedName($this) . ' RENAME COLUMN ' . $oldColumnName->getQuotedName($this) . ' TO ' . $column->getQuotedName($this);
        }
        $fields = [];
        ...
        if (!$this->onSchemaAlterTable($diff, $tableSql)) {
            ...
            if ($newName !== \false) {
                $sql[] = sprintf('ALTER TABLE %s RENAME TO %s', $diff->getName($this)->getQuotedName($this), $newName->getQuotedName($this));
            }
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4749', 'OraclePlatform::getName() is deprecated. Identify platforms by their class.');
        return 'oracle';
    }
    /**
     * {@inheritDoc}
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4510', 'OraclePlatform::getReservedKeywordsClass() is deprecated,' . ' use OraclePlatform::createReservedKeywordsList() instead.');
...
SQL
, $tableCommentsName, $this->quoteStringLiteral($this->normalizeIdentifier($table)->getName()), $ownerCondition);
    }
}
