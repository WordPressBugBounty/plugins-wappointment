...
class SQLServerPlatform extends AbstractPlatform
{
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pulls/1519', 'SQLServerPlatform::prefersIdentityColumns() is deprecated.');
        return \true;
    }
    /**
     * {@inheritDoc}
    ...
    protected function _getCreateTableSQL($name, array $columns, array $options = [])
    {
        ...
        foreach ($columns as &$column) {
            ...
            if (isset($column['default'])) {
                $defaultConstraintsSql[] = 'ALTER TABLE ' . $name . ' ADD' . $this->getDefaultConstraintDeclarationSQL($name, $column);
            }
    ...
    protected function getCreateColumnCommentSQL($tableName, $columnName, $comment)
    {
        ...
        return $this->getAddExtendedPropertySQL('MS_Description', $comment, 'SCHEMA', $schemaSQL, 'TABLE', $tableSQL, 'COLUMN', $columnName);
    }
    /**
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->addedColumns as $column) {
            ...
            if (isset($columnDef['default'])) {
                $addColumnSql .= ' CONSTRAINT ' . $this->generateDefaultConstraintName($diff->name, $column->getQuotedName($this)) . $this->getDefaultValueDeclarationSQL($columnDef);
            }
    ...
    protected function getAlterColumnCommentSQL($tableName, $columnName, $comment)
    {
        ...
        return $this->getUpdateExtendedPropertySQL('MS_Description', $comment, 'SCHEMA', $schemaSQL, 'TABLE', $tableSQL, 'COLUMN', $columnName);
    }
    /**
    ...
    protected function getDropColumnCommentSQL($tableName, $columnName)
    {
        ...
        return $this->getDropExtendedPropertySQL('MS_Description', 'SCHEMA', $schemaSQL, 'TABLE', $tableSQL, 'COLUMN', $columnName);
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        return [sprintf("EXEC sp_rename N'%s.%s', N'%s', N'INDEX'", $tableName, $oldIndexName, $index->getQuotedName($this))];
    }
    /**
    ...
    {
        return 'EXEC sp_addextendedproperty ' . 'N' . $this->quoteStringLiteral($name) . ', N' . $this->quoteStringLiteral((string) $value) . ', ' . 'N' . $this->quoteStringLiteral((string) $level0Type) . ', ' . $level0Name . ', ' . 'N' . $this->quoteStringLiteral((string) $level1Type) . ', ' . $level1Name . ', ' . 'N' . $this->quoteStringLiteral((string) $level2Type) . ', ' . $level2Name;
    }
    /**
    ...
    {
        return 'EXEC sp_dropextendedproperty ' . 'N' . $this->quoteStringLiteral($name) . ', ' . 'N' . $this->quoteStringLiteral((string) $level0Type) . ', ' . $level0Name . ', ' . 'N' . $this->quoteStringLiteral((string) $level1Type) . ', ' . $level1Name . ', ' . 'N' . $this->quoteStringLiteral((string) $level2Type) . ', ' . $level2Name;
    }
    /**
    ...
    {
        return 'EXEC sp_updateextendedproperty ' . 'N' . $this->quoteStringLiteral($name) . ', N' . $this->quoteStringLiteral((string) $value) . ', ' . 'N' . $this->quoteStringLiteral((string) $level0Type) . ', ' . $level0Name . ', ' . 'N' . $this->quoteStringLiteral((string) $level1Type) . ', ' . $level1Name . ', ' . 'N' . $this->quoteStringLiteral((string) $level2Type) . ', ' . $level2Name;
    }
    /**
     * {@inheritDoc}
     */
    ...
    public function getListTableForeignKeysSQL($table, $database = null)
    {
        return 'SELECT f.name AS ForeignKey,
                ...
                WHERE ' . $this->getTableWhereClause($table, 'SCHEMA_NAME (f.schema_id)', 'OBJECT_NAME (f.parent_object_id)') . ' ORDER BY fc.constraint_column_id';
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4503', 'SQLServerPlatform::getListNamespacesSQL() is deprecated,' . ' use SQLServerSchemaManager::listSchemaNames() instead.');
    ...
    {
        return $fixed ? $length > 0 ? 'NCHAR(' . $length . ')' : 'CHAR(255)' : ($length > 0 ? 'NVARCHAR(' . $length . ')' : 'NVARCHAR(255)');
    }
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->doctrineTypeMapping = ['bigint' => 'bigint', 'binary' => 'binary', 'bit' => 'boolean', 'blob' => 'blob', 'char' => 'string', 'date' => 'date', 'datetime' => 'datetime', 'datetime2' => 'datetime', 'datetimeoffset' => 'datetimetz', 'decimal' => 'decimal', 'double' => 'float', 'double precision' => 'float', 'float' => 'float', 'image' => 'blob', 'int' => 'integer', 'money' => 'integer', 'nchar' => 'string', 'ntext' => 'text', 'numeric' => 'decimal', 'nvarchar' => 'string', 'real' => 'float', 'smalldatetime' => 'datetime', 'smallint' => 'smallint', 'smallmoney' => 'integer', 'text' => 'text', 'time' => 'time', 'tinyint' => 'smallint', 'uniqueidentifier' => 'guid', 'varbinary' => 'binary', 'varchar' => 'string'];
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4510', 'SQLServerPlatform::getReservedKeywordsClass() is deprecated,' . ' use SQLServerPlatform::createReservedKeywordsList() instead.');
