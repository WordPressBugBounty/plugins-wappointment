...
abstract class AbstractMySQLPlatform extends AbstractPlatform
{
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pulls/1519', 'AbstractMySQLPlatform::prefersIdentityColumns() is deprecated.');
        return \true;
    }
    /**
     * {@inheritDoc}
...
, $this->getDatabaseNameSQL($database), $this->quoteStringLiteral($table));
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->addedColumns as $column) {
            ...
            $columnArray = array_merge($column->toArray(), ['comment' => $this->getColumnComment($column)]);
    ...
    private function getPreAlterTableAlterIndexForeignKeySQL(TableDiff $diff): array
    {
        ...
        foreach ($diff->changedIndexes as $changedIndex) {
            ...
            foreach ($diff->fromTable->getPrimaryKeyColumns() as $columnName => $column) {
                ...
                if (!$column->getAutoincrement() || in_array($columnName, $changedIndex->getColumns(), \true)) {
                    continue;
                }
    ...
    protected function getPreAlterTableRenameIndexForeignKeySQL(TableDiff $diff)
    {
        ...
        foreach ($this->getRemainingForeignKeyConstraintsRequiringRenamedIndexes($diff) as $foreignKey) {
            if (in_array($foreignKey, $diff->changedForeignKeys, \true)) {
                continue;
            }
    ...
    {
        $this->doctrineTypeMapping = ['bigint' => 'bigint', 'binary' => 'binary', 'blob' => 'blob', 'char' => 'string', 'date' => 'date', 'datetime' => 'datetime', 'decimal' => 'decimal', 'double' => 'float', 'float' => 'float', 'int' => 'integer', 'integer' => 'integer', 'longblob' => 'blob', 'longtext' => 'text', 'mediumblob' => 'blob', 'mediumint' => 'integer', 'mediumtext' => 'text', 'numeric' => 'decimal', 'real' => 'float', 'set' => 'simple_array', 'smallint' => 'smallint', 'string' => 'string', 'text' => 'text', 'time' => 'time', 'timestamp' => 'datetime', 'tinyblob' => 'blob', 'tinyint' => 'boolean', 'tinytext' => 'text', 'varbinary' => 'binary', 'varchar' => 'string', 'year' => 'date'];
    }
    /**
     * {@inheritDoc}
     */
