...
namespace WappoVendor\Doctrine\DBAL\Tools\Console\Command;

use WappoVendor\Doctrine\DBAL\Connection;
use WappoVendor\Doctrine\DBAL\Exception;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\DB2Keywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\KeywordList;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\MariaDb102Keywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\MySQL57Keywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\MySQL80Keywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\MySQLKeywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\OracleKeywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\PostgreSQL100Keywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\PostgreSQL94Keywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\ReservedKeywordsValidator;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\SQLiteKeywords;
use WappoVendor\Doctrine\DBAL\Platforms\Keywords\SQLServer2012Keywords;
use WappoVendor\Doctrine\DBAL\Tools\Console\ConnectionProvider;
use WappoVendor\Doctrine\Deprecations\Deprecation;
use InvalidArgumentException;
use WappoVendor\Symfony\Component\Console\Command\Command;
use WappoVendor\Symfony\Component\Console\Input\InputInterface;
use WappoVendor\Symfony\Component\Console\Input\InputOption;
use WappoVendor\Symfony\Component\Console\Output\OutputInterface;
...
class ReservedWordsCommand extends Command
{
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/issues/4510', 'ReservedWordsCommand::setKeywordListClass() is deprecated,' . ' use ReservedWordsCommand::setKeywordList() instead.');
        $this->keywordLists[$name] = new $class();
    }
    /** @return void */
    protected function configure()
    {
        $this->setName('dbal:reserved-words')->setDescription('Checks if the current database contains identifiers that are reserved.')->setDefinition([new InputOption('connection', null, InputOption::VALUE_REQUIRED, 'The named database connection'), new InputOption('list', 'l', InputOption::VALUE_OPTIONAL | InputOption::VALUE_IS_ARRAY, 'Keyword-List name.')])->setHelp(<<<EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($keywordLists as $keywordList) {
            if (!isset($this->keywordLists[$keywordList])) {
                throw new InvalidArgumentException("There exists no keyword list with name '" . $keywordList . "'. " . 'Known lists: ' . implode(', ', array_keys($this->keywordLists)));
            }
