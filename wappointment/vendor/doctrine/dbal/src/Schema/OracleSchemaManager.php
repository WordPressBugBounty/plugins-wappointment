...
class OracleSchemaManager extends AbstractSchemaManager
{
    ...
    protected function _getPortableTableColumnDefinition($tableColumn)
    {
        ...
        $options = ['notnull' => $tableColumn['nullable'] === 'N', 'fixed' => (bool) $fixed, 'unsigned' => (bool) $unsigned, 'default' => $tableColumn['data_default'], 'length' => $length, 'precision' => $precision, 'scale' => $scale, 'comment' => isset($tableColumn['comments']) && $tableColumn['comments'] !== '' ? $tableColumn['comments'] : null];
    ...
    protected function _getPortableTableForeignKeysList($tableForeignKeys)
    {
        ...
        foreach ($tableForeignKeys as $value) {
            ...
            if (!isset($list[$value['constraint_name']])) {
                ...
                $list[$value['constraint_name']] = ['name' => $this->getQuotedIdentifierName($value['constraint_name']), 'local' => [], 'foreign' => [], 'foreignTable' => $value['references_table'], 'onDelete' => $value['delete_rule']];
            }
        ...
        foreach ($list as $constraint) {
            $result[] = new ForeignKeyConstraint(array_values($constraint['local']), $this->getQuotedIdentifierName($constraint['foreignTable']), array_values($constraint['foreign']), $this->getQuotedIdentifierName($constraint['name']), ['onDelete' => $constraint['onDelete']]);
        }
        return $result;
    }
    /**
