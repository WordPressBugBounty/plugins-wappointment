...
use WappoVendor\Doctrine\DBAL\Platforms\SQLite;
use WappoVendor\Doctrine\DBAL\Platforms\SqlitePlatform;
use WappoVendor\Doctrine\DBAL\Types\StringType;
use WappoVendor\Doctrine\DBAL\Types\TextType;
...
class SqliteSchemaManager extends AbstractSchemaManager
{
    ...
    public function listTableForeignKeys($table, $database = null)
    {
        ...
        if (!empty($tableForeignKeys)) {
            ...
            foreach ($tableForeignKeys as $key => $value) {
                $id = $value['id'];
                $tableForeignKeys[$key] = array_merge($tableForeignKeys[$key], ['constraint_name' => isset($names[$id]) && $names[$id] !== '' ? $names[$id] : $id, 'deferrable' => isset($deferrable[$id]) && strtolower($deferrable[$id]) === 'deferrable', 'deferred' => isset($deferred[$id]) && strtolower($deferred[$id]) === 'deferred']);
            }
        }
    ...
    protected function _getPortableTableColumnDefinition($tableColumn)
    {
        ...
        $options = ['length' => $length, 'unsigned' => $unsigned, 'fixed' => $fixed, 'notnull' => $notnull, 'default' => $default, 'precision' => $precision, 'scale' => $scale, 'autoincrement' => \false];
    ...
    protected function _getPortableTableForeignKeysList($tableForeignKeys)
    {
        ...
        foreach ($list as $constraint) {
            $result[] = new ForeignKeyConstraint($constraint['local'], $constraint['foreignTable'], $constraint['foreign'], $constraint['name'], ['onDelete' => $constraint['onDelete'], 'onUpdate' => $constraint['onUpdate'], 'deferrable' => $constraint['deferrable'], 'deferred' => $constraint['deferred']]);
        }
        return $result;
    }
    /**
...
, [$table]);
    ...
    {
        Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/4821', 'SqliteSchemaManager::getSchemaSearchPaths() is deprecated.');
