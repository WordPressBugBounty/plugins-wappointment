...
class Comparator
{
    ...
    {
        if ($platform === null) {
            Deprecation::triggerIfCalledFromOutside('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/4746', 'Not passing a $platform to %s is deprecated.' . ' Use AbstractSchemaManager::createComparator() to instantiate the comparator.', __METHOD__);
        }
     ...
     */
    private function doCompareSchemas(Schema $fromSchema, Schema $toSchema)
    {
        $diff = new SchemaDiff();
    ...
    {
        Deprecation::trigger('doctrine/dbal', 'https://github.com/doctrine/dbal/pull/4707', 'Method compare() is deprecated. Use a non-static call to compareSchemas() instead.');
    ...
    public function diffTable(Table $fromTable, Table $toTable)
    {
        ...
        foreach ($fromTableColumns as $columnName => $column) {
            ...
            $tableDifferences->changedColumns[$column->getName()] = new ColumnDiff($column->getName(), $toColumn, $changedProperties, $column);
            $changes++;
        }
        ...
        foreach ($fromTableIndexes as $indexName => $index) {
            ...
            if ($index->isPrimary() && !$toTable->hasPrimaryKey() || !$index->isPrimary() && !$toTable->hasIndex($indexName)) {
    ...
    private function detectColumnRenamings(TableDiff $tableDifferences): void
    {
        ...
        foreach ($renameCandidates as $candidateColumns) {
            ...
            unset($tableDifferences->addedColumns[$addedColumnName], $tableDifferences->removedColumns[strtolower($removedColumnName)]);
        }
    }
    /**
    ...
    private function detectIndexRenamings(TableDiff $tableDifferences): void
    {
        ...
        foreach ($renameCandidates as $candidateIndexes) {
            ...
            unset($tableDifferences->addedIndexes[$addedIndexName], $tableDifferences->removedIndexes[$removedIndexName]);
        }
    }
    /**
     * @return bool
    ...
    public function diffColumn(Column $column1, Column $column2)
    {
        ...
        if ($properties1['comment'] !== $properties2['comment'] && !($properties1['comment'] === null && $properties2['comment'] === '') && !($properties2['comment'] === null && $properties1['comment'] === '')) {
