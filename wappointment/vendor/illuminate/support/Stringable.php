...
class Stringable implements JsonSerializable
{
    ...
    {
        return new static(\WappointmentLv::class_basename($this->value));
    }
    /**
    ...
    {
        return \WappointmentLv::collect(explode($delimiter, $this->value, $limit));
    }
    /**
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (filter_var($pattern, \FILTER_VALIDATE_INT) !== \false) {
            return \WappointmentLv::collect(mb_str_split($this->value, $pattern));
        }
        $segments = preg_split($pattern, $this->value, $limit, $flags);
        return !empty($segments) ? \WappointmentLv::collect($segments) : \WappointmentLv::collect();
    }
    /**
    ...
    {
        return \WappointmentLv::collect(sscanf($this->value, $format));
    }
    /**
    ...
    {
        return \WappointmentLv::collect(Str::ucsplit($this->value));
    }
    /**
