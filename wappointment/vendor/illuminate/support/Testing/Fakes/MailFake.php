...
use Closure;
use WappoVendor\Illuminate\Contracts\Mail\Factory;
use WappoVendor\Illuminate\Contracts\Mail\Mailable;
use WappoVendor\Illuminate\Contracts\Mail\Mailer;
use WappoVendor\Illuminate\Contracts\Mail\MailQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = \WappointmentLv::collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = \WappointmentLv::collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
