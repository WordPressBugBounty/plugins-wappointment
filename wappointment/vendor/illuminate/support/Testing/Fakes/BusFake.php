...
use Closure;
use WappoVendor\Illuminate\Bus\PendingBatch;
use WappoVendor\Illuminate\Contracts\Bus\QueueingDispatcher;
use WappoVendor\Illuminate\Support\Arr;
use WappoVendor\Illuminate\Support\Collection;
use WappoVendor\Illuminate\Support\Traits\ReflectsClosures;
...
class BusFake implements QueueingDispatcher
{
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(\WappointmentLv::collect($expectedChain)->isNotEmpty(), 'The expected chain can not be empty.');
    ...
    {
        return \WappointmentLv::tap(clone $job, function ($job) {
    ...
    {
        $chain = \WappointmentLv::collect($expectedChain)->map(function ($job) {
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return \WappointmentLv::collect($chain)->map(function ($job) {
    ...
    {
        return !\WappointmentLv::collect($chain)->contains(function ($job) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return \WappointmentLv::collect($this->commands[$command])->filter(function ($command) use ($callback) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return \WappointmentLv::collect($this->commandsSync[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    /**
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return \WappointmentLv::collect($this->commandsAfterResponse[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    /**
    ...
    public function batched(callable $callback)
    {
        ...
        return \WappointmentLv::collect($this->batches)->filter(function ($batch) use ($callback) {
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return \WappointmentLv::collect($this->jobsToFake)->filter(function ($job) use ($command) {
