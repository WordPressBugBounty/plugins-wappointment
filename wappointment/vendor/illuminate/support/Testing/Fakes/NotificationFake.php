...
use Closure;
use Exception;
use WappoVendor\Illuminate\Contracts\Notifications\Dispatcher as NotificationDispatcher;
use WappoVendor\Illuminate\Contracts\Notifications\Factory as NotificationFactory;
use WappoVendor\Illuminate\Contracts\Translation\HasLocalePreference;
...
class NotificationFake implements NotificationDispatcher, NotificationFactory
{
    ...
    {
        $actualCount = \WappointmentLv::collect($this->notifications)->flatten(1)->reduce(function ($count, $sent) use ($notification) {
    ...
    public function sent($notifiable, $notification, $callback = null)
    {
        ...
        $notifications = \WappointmentLv::collect($this->notificationsFor($notifiable, $notification));
    ...
    public function sendNow($notifiables, $notification, array $channels = null)
    {
        ...
        foreach ($notifiables as $notifiable) {
            ...
            $this->notifications[get_class($notifiable)][$notifiable->getKey()][get_class($notification)][] = ['notification' => $notification, 'channels' => $notifiableChannels, 'notifiable' => $notifiable, 'locale' => $notification->locale ?? $this->locale ?? \WappointmentLv::value(function () use ($notifiable) {
