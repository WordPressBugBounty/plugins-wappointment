...
class Builder
{
    ...
    {
        $this->build(\WappointmentLv::tap($this->createBlueprint($table), function ($blueprint) use ($callback) {
            $blueprint->create();
            $callback($blueprint);
        }));
    }
    ...
    {
        $this->build(\WappointmentLv::tap($this->createBlueprint($table), function ($blueprint) {
            $blueprint->drop();
        }));
    }
    /**
    ...
    {
        $this->build(\WappointmentLv::tap($this->createBlueprint($from), function ($blueprint) use ($to) {
            $blueprint->rename($to);
        }));
    }
    /**
