...
namespace WappoVendor\Illuminate\Database\Eloquent\Concerns;

use WappoVendor\Carbon\CarbonImmutable;
use WappoVendor\Carbon\CarbonInterface;
use DateTimeInterface;
use WappoVendor\Illuminate\Contracts\Database\Eloquent\Castable;
use WappoVendor\Illuminate\Contracts\Database\Eloquent\CastsInboundAttributes;
use WappoVendor\Illuminate\Contracts\Support\Arrayable;
use WappoVendor\Illuminate\Database\Eloquent\Casts\AsArrayObject;
use WappoVendor\Illuminate\Database\Eloquent\Casts\AsCollection;
use WappoVendor\Illuminate\Database\Eloquent\Casts\Attribute;
use WappoVendor\Illuminate\Database\Eloquent\InvalidCastException;
use WappoVendor\Illuminate\Database\Eloquent\JsonEncodingException;
use WappoVendor\Illuminate\Database\Eloquent\Relations\Relation;
use WappoVendor\Illuminate\Database\LazyLoadingViolationException;
use WappoVendor\Illuminate\Support\Arr;
use WappoVendor\Illuminate\Support\Carbon;
use WappoVendor\Illuminate\Support\Collection as BaseCollection;
use WappoVendor\Illuminate\Support\Facades\Crypt;
use WappoVendor\Illuminate\Support\Facades\Date;
...
trait HasAttributes
{
    ...
    public function getAttribute($key)
    {
        ...
        if (array_key_exists($key, $this->attributes) || array_key_exists($key, $this->casts) || $this->hasGetMutator($key) || $this->hasAttributeMutator($key) || $this->isClassCastable($key)) {
    ...
    protected function getRelationshipFromMethod($method)
    {
        ...
        return \WappointmentLv::tap($relation->getResults(), function ($results) use ($method) {
    ...
    {
        return \WappointmentLv::tap($this->getArrayAttributeByKey($key), function (&$array) use ($path, $value) {
    ...
    protected function getOriginalWithoutRewindingModel($key = null, $default = null)
    {
        ...
        return \WappointmentLv::collect($this->original)->mapWithKeys(function ($value, $key) {
    ...
    {
        static::$getAttributeMutatorCache[$class] = \WappointmentLv::collect($attributeMutatorMethods = static::getAttributeMarkedMutatorMethods($class))->mapWithKeys(function ($match) {
            return [lcfirst(static::$snakeAttributes ? Str::snake($match) : $match) => \true];
        })->all();
        static::$mutatorCache[$class] = \WappointmentLv::collect(static::getMutatorMethods($class))->merge($attributeMutatorMethods)->map(function ($match) {
    ...
    protected static function getAttributeMarkedMutatorMethods($class)
    {
        ...
        return \WappointmentLv::collect((new ReflectionClass($instance))->getMethods())->filter(function ($method) use ($instance) {
