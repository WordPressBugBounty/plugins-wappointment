...
use WappoVendor\Illuminate\Database\Query\Builder as QueryBuilder;
use WappoVendor\Illuminate\Database\RecordsNotFoundException;
...
class Builder
{
     ...
     * @var string[]
     */
    protected $passthru = ['aggregate', 'average', 'avg', 'count', 'dd', 'doesntExist', 'dump', 'exists', 'explain', 'getBindings', 'getConnection', 'getGrammar', 'insert', 'insertGetId', 'insertOrIgnore', 'insertUsing', 'max', 'min', 'raw', 'sum', 'toSql'];
    /**
    ...
    public function firstOrCreate(array $attributes = [], array $values = [])
    {
        ...
        return \WappointmentLv::tap($this->newModelInstance(array_merge($attributes, $values)), function ($instance) {
            $instance->save();
        });
    }
    /**
    ...
    {
        return \WappointmentLv::tap($this->firstOrNew($attributes), function ($instance) use ($values) {
    ...
    protected function ensureOrderForCursorPagination($shouldReverse = \false)
    {
        ...
        if ($shouldReverse) {
            $this->query->orders = \WappointmentLv::collect($this->query->orders)->map(function ($order) {
                $order['direction'] = $order['direction'] === 'asc' ? 'desc' : 'asc';
                return $order;
            })->toArray();
        }
        if ($this->query->unionOrders) {
            return \WappointmentLv::collect($this->query->unionOrders);
        }
        return \WappointmentLv::collect($this->query->orders);
    }
    /**
    ...
    {
        return \WappointmentLv::tap($this->newModelInstance($attributes), function ($instance) {
            $instance->save();
        });
    }
    /**
    ...
    {
        $whereBooleans = \WappointmentLv::collect($whereSlice)->pluck('boolean');
