...
use ArrayAccess;
use WappoVendor\Illuminate\Contracts\Broadcasting\HasBroadcastChannel;
use WappoVendor\Illuminate\Contracts\Queue\QueueableCollection;
use WappoVendor\Illuminate\Contracts\Queue\QueueableEntity;
use WappoVendor\Illuminate\Contracts\Routing\UrlRoutable;
use WappoVendor\Illuminate\Contracts\Support\Arrayable;
use WappoVendor\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString;
...
use WappoVendor\Illuminate\Database\Eloquent\Collection as EloquentCollection;
use WappoVendor\Illuminate\Database\Eloquent\Relations\BelongsToMany;
use WappoVendor\Illuminate\Database\Eloquent\Relations\Concerns\AsPivot;
use WappoVendor\Illuminate\Database\Eloquent\Relations\HasManyThrough;
use WappoVendor\Illuminate\Database\Eloquent\Relations\Pivot;
use WappoVendor\Illuminate\Support\Arr;
use WappoVendor\Illuminate\Support\Collection as BaseCollection;
use WappoVendor\Illuminate\Support\Str;
use WappoVendor\Illuminate\Support\Traits\ForwardsCalls;
...
abstract class Model implements Arrayable, ArrayAccess, CanBeEscapedWhenCastToString, HasBroadcastChannel, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    protected static function bootTraits()
    {
        ...
        foreach (\WappointmentLv::class_uses_recursive($class) as $trait) {
            $method = 'boot' . \WappointmentLv::class_basename($trait);
            if (method_exists($class, $method) && !in_array($method, $booted)) {
                forward_static_call([$class, $method]);
                $booted[] = $method;
            }
            if (method_exists($class, $method = 'initialize' . \WappointmentLv::class_basename($trait))) {
    ...
    {
        return \WappointmentLv::collect($columns)->map(function ($column) {
    ...
    protected function incrementOrDecrement($column, $amount, $extra, $method)
    {
        ...
        return \WappointmentLv::tap($this->setKeysForSaveQuery($query)->{$method}($column, $amount, $extra), function () use ($column) {
            $this->syncChanges();
    ...
    public function refresh()
    {
        ...
        $this->load(\WappointmentLv::collect($this->relations)->reject(function ($relation) {
            return $relation instanceof Pivot || is_object($relation) && in_array(AsPivot::class, \WappointmentLv::class_uses_recursive($relation), \true);
        })->keys()->all());
        $this->syncOriginal();
        return $this;
    }
    ...
    public function replicate(array $except = null)
    {
        ...
        return \WappointmentLv::tap(new static(), function ($instance) use ($attributes) {
    ...
    {
        return $this->table ?? Str::snake(Str::pluralStudly(\WappointmentLv::class_basename($this)));
    }
    /**
    ...
    {
        return Str::snake(\WappointmentLv::class_basename($this)) . '_' . $this->getKeyName();
    }
    /**
    ...
    {
        return str_replace('\\', '.', get_class($this)) . '.{' . Str::camel(\WappointmentLv::class_basename($this)) . '}';
    }
    /**
