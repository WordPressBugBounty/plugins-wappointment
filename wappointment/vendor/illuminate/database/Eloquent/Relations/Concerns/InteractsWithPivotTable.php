...
namespace WappoVendor\Illuminate\Database\Eloquent\Relations\Concerns;
...
trait InteractsWithPivotTable
{
    ...
    {
        return $this->sync(\WappointmentLv::collect($this->parseIds($ids))->mapWithKeys(function ($id) use ($values) {
            return [$id => $values];
        }), $detaching);
    }
    /**
    ...
    {
        return \WappointmentLv::collect($records)->mapWithKeys(function ($attributes, $id) {
    ...
    {
        $pivot = $this->getCurrentlyAttachedPivots()->where($this->foreignPivotKey, $this->parent->{$this->parentKey})->where($this->relatedPivotKey, $this->parseId($id))->first();
     ...
     * @return void
     */
    public function attach($id, array $attributes = [], $touch = \true)
    {
        if ($this->using) {
    ...
    {
        $records = $this->formatAttachRecords($this->parseIds($id), $attributes);
    ...
    protected function detachUsingCustomClass($ids)
    {
        ...
        foreach ($this->parseIds($ids) as $id) {
            $results += $this->newPivot([$this->foreignPivotKey => $this->parent->{$this->parentKey}, $this->relatedPivotKey => $id], \true)->delete();
        }
        return $results;
    }
    /**
