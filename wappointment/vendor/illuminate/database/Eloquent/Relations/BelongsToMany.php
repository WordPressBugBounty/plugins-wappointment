...
use WappoVendor\Illuminate\Database\Eloquent\ModelNotFoundException;
use WappoVendor\Illuminate\Database\Eloquent\Relations\Concerns\AsPivot;
use WappoVendor\Illuminate\Database\Eloquent\Relations\Concerns\InteractsWithDictionary;
use WappoVendor\Illuminate\Database\Eloquent\Relations\Concerns\InteractsWithPivotTable;
...
class BelongsToMany extends Relation
{
    ...
    protected function resolveTableName($table)
    {
        ...
        if (in_array(AsPivot::class, \WappointmentLv::class_uses_recursive($model))) {
            $this->using($table);
        }
    ...
    protected function aliasedPivotColumns()
    {
        ...
        return \WappointmentLv::collect(array_merge($defaults, $this->pivotColumns))->map(function ($column) {
    ...
    public function paginate($perPage = null, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        return \WappointmentLv::tap($this->query->paginate($perPage, $columns, $pageName, $page), function ($paginator) {
    ...
    {
        $this->query->addSelect($this->shouldSelect($columns));
        return \WappointmentLv::tap($this->query->simplePaginate($perPage, $columns, $pageName, $page), function ($paginator) {
            $this->hydratePivotRelation($paginator->items());
        });
    }
    /**
    ...
    {
        $this->query->addSelect($this->shouldSelect($columns));
        return \WappointmentLv::tap($this->query->cursorPaginate($perPage, $columns, $cursorName, $cursor), function ($paginator) {
            $this->hydratePivotRelation($paginator->items());
        });
    }
    /**
    ...
    {
        return Str::camel(Str::pluralStudly(\WappointmentLv::class_basename($this->getParent())));
    }
    /**
