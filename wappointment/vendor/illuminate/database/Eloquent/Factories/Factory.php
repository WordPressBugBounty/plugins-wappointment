...
use Closure;
use WappoVendor\Faker\Generator;
...
abstract class Factory
{
    ...
    {
        return new EloquentCollection(\WappointmentLv::collect($records)->map(function ($record) {
    ...
    public function create($attributes = [], ?Model $parent = null)
    {
        ...
        if ($results instanceof Model) {
            $this->store(\WappointmentLv::collect([$results]));
            $this->callAfterCreating(\WappointmentLv::collect([$results]), $parent);
        } else {
            $this->store($results);
    ...
    public function make($attributes = [], ?Model $parent = null)
    {
        ...
        if ($this->count === null) {
            return \WappointmentLv::tap($this->makeInstance($parent), function ($instance) {
                $this->callAfterMaking(\WappointmentLv::collect([$instance]));
            });
        }
        if ($this->count < 1) {
    ...
    protected function makeInstance(?Model $parent)
    {
        return Model::unguarded(function () use ($parent) {
            return \WappointmentLv::tap($this->newModel($this->getExpandedAttributes($parent)), function ($instance) {
                if (isset($this->connection)) {
    ...
    {
        return \WappointmentLv::collect($definition)->map(function ($attribute, $key) use (&$definition) {
    ...
    {
        return $this->newInstance(['states' => $this->states->concat([is_callable($state) ? $state : function () use ($state) {
            return $state;
        }])]);
    }
    /**
    ...
    {
        $guess = Str::camel(Str::plural(\WappointmentLv::class_basename($related)));
    ...
    {
        return $this->newInstance(['has' => $this->has->concat([new BelongsToManyRelationship($factory, $pivot, $relationship ?: Str::camel(Str::plural(\WappointmentLv::class_basename($factory instanceof Factory ? $factory->modelName() : Collection::wrap($factory)->first()))))])]);
    }
    /**
    ...
    {
        return $this->newInstance(['for' => $this->for->concat([new BelongsToRelationship($factory, $relationship ?: Str::camel(\WappointmentLv::class_basename($factory instanceof Factory ? $factory->modelName() : $factory)))])]);
    }
    /**
    ...
    {
        return new static(...array_values(array_merge(['count' => $this->count, 'states' => $this->states, 'has' => $this->has, 'for' => $this->for, 'afterMaking' => $this->afterMaking, 'afterCreating' => $this->afterCreating, 'connection' => $this->connection], $arguments)));
    }
    /**
    ...
    public function modelName()
    {
        $resolver = static::$modelNameResolver ?: function (self $factory) {
            $namespacedFactoryBasename = Str::replaceLast('Factory', '', Str::replaceFirst(static::$namespace, '', get_class($factory)));
            $factoryBasename = Str::replaceLast('Factory', '', \WappointmentLv::class_basename($factory));
