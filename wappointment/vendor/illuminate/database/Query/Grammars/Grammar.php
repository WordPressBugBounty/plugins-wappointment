...
namespace WappoVendor\Illuminate\Database\Query\Grammars;

use WappoVendor\Illuminate\Database\Grammar as BaseGrammar;
use WappoVendor\Illuminate\Database\Query\Builder;
use WappoVendor\Illuminate\Database\Query\JoinClause;
...
class Grammar extends BaseGrammar
{
    ...
    {
        return \WappointmentLv::collect($joins)->map(function ($join) use ($query) {
    ...
    {
        return \WappointmentLv::collect($query->wheres)->map(function ($where) use ($query) {
    ...
    protected function compileHavingBetween($having)
    {
        ...
        $column = $this->wrap($having['column']);
        $min = $this->parameter(\WappointmentLv::head($having['values']));
        $max = $this->parameter(\WappointmentLv::last($having['values']));
    ...
    public function compileInsert(Builder $query, array $values)
    {
        ...
        $parameters = \WappointmentLv::collect($values)->map(function ($record) {
    ...
    {
        return \WappointmentLv::collect($values)->map(function ($value, $key) {
    ...
    {
        $alias = \WappointmentLv::last(explode(' as ', $table));
        $joins = $this->compileJoins($query, $query->joins);
