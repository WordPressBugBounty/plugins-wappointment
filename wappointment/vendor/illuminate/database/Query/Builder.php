...
namespace WappoVendor\Illuminate\Database\Query;

use BackedEnum;
use Closure;
use DateTimeInterface;
use WappoVendor\Illuminate\Contracts\Support\Arrayable;
use WappoVendor\Illuminate\Database\Concerns\BuildsQueries;
use WappoVendor\Illuminate\Database\Concerns\ExplainsQueries;
use WappoVendor\Illuminate\Database\ConnectionInterface;
use WappoVendor\Illuminate\Database\Eloquent\Builder as EloquentBuilder;
use WappoVendor\Illuminate\Database\Eloquent\Relations\Relation;
use WappoVendor\Illuminate\Database\Query\Grammars\Grammar;
use WappoVendor\Illuminate\Database\Query\Processors\Processor;
use WappoVendor\Illuminate\Pagination\Paginator;
use WappoVendor\Illuminate\Support\Arr;
use WappoVendor\Illuminate\Support\Collection;
use WappoVendor\Illuminate\Support\LazyCollection;
...
class Builder
{
    ...
    {
        return \WappointmentLv::collect($this->onceWithColumns(Arr::wrap($columns), function () {
    ...
    public function paginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        $results = $total ? $this->forPage($page, $perPage)->get($columns) : \WappointmentLv::collect();
    ...
    {
        $this->enforceOrderBy();
        return \WappointmentLv::collect($this->orders ?? $this->unionOrders ?? [])->filter(function ($order) {
    ...
    protected function stripTableForPluck($column)
    {
        ...
        return \WappointmentLv::last(preg_split('~' . $separator . '~i', $column));
    }
    /**
    ...
    protected function pluckFromObjectColumn($queryResult, $column, $key)
    {
        ...
        return \WappointmentLv::collect($results);
    }
    /**
    ...
    public function upsert(array $values, $uniqueBy, $update = null)
    {
        ...
        $this->applyBeforeQueryCallbacks();
        $bindings = $this->cleanBindings(array_merge(Arr::flatten($values, 1), \WappointmentLv::collect($update)->reject(function ($value, $key) {
            return is_int($key);
        })->all()));
    ...
    {
        return \WappointmentLv::collect($bindings)->reject(function ($binding) {
    ...
    {
        return is_array($value) ? \WappointmentLv::head(Arr::flatten($value)) : $value;
    }
    /**
    ...
    {
        return \WappointmentLv::tap($this->clone(), function ($clone) use ($properties) {
    ...
    {
        return \WappointmentLv::tap($this->clone(), function ($clone) use ($except) {
