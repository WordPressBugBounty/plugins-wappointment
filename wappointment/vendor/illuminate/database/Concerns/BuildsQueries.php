...
use WappoVendor\Illuminate\Database\MultipleRecordsFoundException;
use WappoVendor\Illuminate\Database\RecordsNotFoundException;
use WappoVendor\Illuminate\Pagination\Cursor;
use WappoVendor\Illuminate\Pagination\CursorPaginator;
use WappoVendor\Illuminate\Pagination\LengthAwarePaginator;
    ...
    protected function paginateUsingCursor($perPage, $columns = ['*'], $cursorName = 'cursor', $cursor = null)
    {
        ...
        if (!is_null($cursor)) {
            $addCursorConditions = function (self $builder, $previousColumn, $i) use (&$addCursorConditions, $cursor, $orders) {
                $unionBuilders = isset($builder->unions) ? \WappointmentLv::collect($builder->unions)->pluck('query') : \WappointmentLv::collect();
                ...
                $builder->where(function (self $builder) use ($addCursorConditions, $cursor, $orders, $i, $unionBuilders) {
                    ...
                    $unionBuilders->each(function ($unionBuilder) use ($column, $direction, $cursor, $i, $orders, $addCursorConditions) {
                        $unionBuilder->where(function ($unionBuilder) use ($column, $direction, $cursor, $i, $orders, $addCursorConditions) {
                            $unionBuilder->where($this->getOriginalColumnNameForCursorPagination($this, $column), $direction === 'asc' ? '>' : '<', $cursor->parameter($column));
                            if ($i < $orders->count() - 1) {
