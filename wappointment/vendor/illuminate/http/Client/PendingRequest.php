...
use WappoVendor\GuzzleHttp\Client;
use WappoVendor\GuzzleHttp\Cookie\CookieJar;
use WappoVendor\GuzzleHttp\Exception\ConnectException;
use WappoVendor\GuzzleHttp\Exception\RequestException;
use WappoVendor\GuzzleHttp\Exception\TransferException;
use WappoVendor\GuzzleHttp\HandlerStack;
use WappoVendor\Illuminate\Http\Client\Events\ConnectionFailed;
use WappoVendor\Illuminate\Http\Client\Events\RequestSending;
use WappoVendor\Illuminate\Http\Client\Events\ResponseReceived;
...
use WappoVendor\Illuminate\Support\Traits\Conditionable;
use WappoVendor\Illuminate\Support\Traits\Macroable;
use WappoVendor\Psr\Http\Message\MessageInterface;
use WappoVendor\Psr\Http\Message\RequestInterface;
...
class PendingRequest
{
     ...
     * @var array
     */
    protected $mergableOptions = ['cookies', 'form_params', 'headers', 'json', 'multipart', 'query'];
    /**
    ...
    public function __construct(Factory $factory = null)
    {
        ...
        $this->beforeSendingCallbacks = \WappointmentLv::collect([function (Request $request, array $options, PendingRequest $pendingRequest) {
    ...
    {
        return \WappointmentLv::tap($this, function ($request) {
    ...
    {
        return $this->send('PUT', $url, [$this->bodyFormat => $data]);
    }
    /**
    ...
    {
        return $this->send('DELETE', $url, empty($data) ? [] : [$this->bodyFormat => $data]);
    }
    /**
    ...
    {
        $results = [];
        $requests = \WappointmentLv::tap(new Pool($this->factory), $callback)->getRequests();
    ...
    protected function makePromise(string $method, string $url, array $options = [])
    {
        return $this->promise = $this->sendRequest($method, $url, $options)->then(function (MessageInterface $message) {
            return \WappointmentLv::tap(new Response($message), function ($response) {
                $this->populateResponse($response);
                $this->dispatchResponseReceivedEvent($response);
            });
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        return $this->buildClient()->{$clientMethod}($method, $url, $this->mergeOptions(['laravel_data' => $laravelData, 'on_stats' => function ($transferStats) {
    ...
    {
        return $this->requestsReusableClient() ? $this->getReusableClient() : $this->createClient($this->buildHandlerStack());
    }
    /**
    ...
    {
        return new Client(['handler' => $handlerStack, 'cookies' => \true]);
    }
    /**
    ...
    {
        return \WappointmentLv::tap($handlerStack, function ($stack) {
    ...
    {
        $this->stubCallbacks = \WappointmentLv::collect($callback);
        return $this;
    }
    /**
     ...
     * @return $this
     */
    public function async(bool $async = \true)
    {
        $this->async = $async;
        return $this;
    }
    ...
    {
        if (!($dispatcher = optional($this->factory)->getDispatcher()) || !$this->request) {
            return;
        }
