...
use WappoVendor\Symfony\Component\HttpFoundation\ParameterBag;
use WappoVendor\Symfony\Component\HttpFoundation\Request as SymfonyRequest;
/**
...
{
    use Concerns\InteractsWithContentTypes, Concerns\InteractsWithFlashData, Concerns\InteractsWithInput, Macroable;
    /**
    ...
    {
        return $this->headers->get('X-PJAX') == \true;
    }
    /**
    ...
    {
        return $this->merge(\WappointmentLv::collect($input)->filter(function ($value, $key) {
    ...
    {
        $newRequest = (new static())->duplicate($request->query->all(), $request->request->all(), $request->attributes->all(), $request->cookies->all(), $request->files->all(), $request->server->all());
