...
use WappoVendor\Illuminate\Http\Resources\Json\JsonResource;
...
use WappoVendor\Illuminate\Support\Traits\Tappable;
/**
...
abstract class AbstractCursorPaginator implements Htmlable
{
    ...
    {
        if (is_null($this->cursor) && !$this->hasMore || !is_null($this->cursor) && $this->cursor->pointsToNextItems() && !$this->hasMore) {
            return null;
        }
        if ($this->items->isEmpty()) {
            return null;
        }
        return $this->getCursorForItem($this->items->last(), \true);
    }
    /**
    ...
    {
        return \WappointmentLv::collect($this->parameters)->flip()->map(function ($_, $parameterName) use ($item) {
            if ($item instanceof JsonResource) {
                $item = $item->resource;
            }
            if ($item instanceof Model && !is_null($parameter = $this->getPivotParameterForItem($item, $parameterName))) {
                return $parameter;
            } elseif ($item instanceof ArrayAccess || is_array($item)) {
                return $this->ensureParameterIsPrimitive($item[$parameterName] ?? $item[Str::afterLast($parameterName, '.')]);
    ...
    protected function getPivotParameterForItem($item, $parameterName)
    {
        ...
        foreach ($item->getRelations() as $relation) {
            if ($relation instanceof Pivot && $relation->getTable() === $table) {
                return $this->ensureParameterIsPrimitive($relation->getAttribute(Str::afterLast($parameterName, '.')));
            }
        }
    }
    /**
    ...
    {
        return is_object($parameter) && method_exists($parameter, '__toString') ? (string) $parameter : $parameter;
    }
    /**
