...
use ArrayAccess;
use Countable;
use WappoVendor\Illuminate\Contracts\Pagination\LengthAwarePaginator as LengthAwarePaginatorContract;
...
class LengthAwarePaginator extends AbstractPaginator implements Arrayable, ArrayAccess, Countable, IteratorAggregate, Jsonable, JsonSerializable, LengthAwarePaginatorContract
{
    ...
    {
        return \WappointmentLv::collect($this->elements())->flatMap(function ($item) {
            if (!is_array($item)) {
                return [['url' => null, 'label' => '...', 'active' => \false]];
            }
            return \WappointmentLv::collect($item)->map(function ($url, $page) {
                return ['url' => $url, 'label' => (string) $page, 'active' => $this->currentPage() === $page];
            });
        })->prepend(['url' => $this->previousPageUrl(), 'label' => function_exists('WappoVendor\__') ? __('pagination.previous') : 'Previous', 'active' => \false])->push(['url' => $this->nextPageUrl(), 'label' => function_exists('WappoVendor\__') ? __('pagination.next') : 'Next', 'active' => \false]);
    }
    /**
