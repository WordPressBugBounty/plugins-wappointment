...
namespace WappoVendor\Illuminate\Support\Traits;
...
use WappoVendor\Illuminate\Support\Enumerable;
use WappoVendor\Illuminate\Support\HigherOrderCollectionProxy;
use WappoVendor\Illuminate\Support\HigherOrderWhenProxy;
...
trait EnumeratesValues
{
    ...
    public function containsStrict($key, $value = null)
    {
        if (func_num_args() === 2) {
            return $this->contains(function ($item) use ($key, $value) {
                return \WappointmentLv::data_get($item, $key) === $value;
            });
        }
    ...
    public function whereIn($key, $values, $strict = \false)
    {
        ...
        return $this->filter(function ($item) use ($key, $values, $strict) {
            return in_array(\WappointmentLv::data_get($item, $key), $values, $strict);
        });
    }
    /**
     * Filter items by the given key value pair using strict comparison.
    ...
    public function whereNotBetween($key, $values)
    {
        return $this->filter(function ($item) use ($key, $values) {
            return \WappointmentLv::data_get($item, $key) < reset($values) || \WappointmentLv::data_get($item, $key) > end($values);
        });
    }
    /**
     * Filter items by the given key value pair.
    ...
    public function reduceSpread(callable $callback, ...$initial)
    {
        ...
        foreach ($this as $key => $value) {
            ...
            if (!is_array($result)) {
                throw new UnexpectedValueException(sprintf("%s::reduceMany expects reducer to return an array, but got a '%s' instead.", \WappointmentLv::class_basename(static::class), gettype($result)));
            }
        }
        return $result;
    }
    ...
    protected function operatorForWhere($key, $operator = null, $value = null)
    {
        ...
        return function ($item) use ($key, $operator, $value) {
            $retrieved = \WappointmentLv::data_get($item, $key);
    ...
    protected function valueRetriever($value)
    {
        ...
        return function ($item) use ($value) {
            return \WappointmentLv::data_get($item, $value);
        };
    }
    /**
